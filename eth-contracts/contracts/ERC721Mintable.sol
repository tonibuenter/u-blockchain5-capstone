...
contract Ownable {
    ...
    event OwnershipIsTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () internal {
        _owner = msg.sender;
        emit OwnershipIsTransferred(address(0), _owner);
    }

    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), 'EMPTY_NEW_OWNER');
        require(newOwner != _owner, 'SAME_OWNER');
        emit OwnershipIsTransferred(_owner, newOwner);
        _owner = newOwner;
    }
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused(){
        require(_paused == false, 'CONTRACT_IS_PAUSED');
        _;
    }

    modifier paused(){
        require(_paused == true, 'CONTRACT_IS_NOT_PAUSED');
        _;
    }
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 _tokenId) public {
        ...
        require(to != tokenOwner, "CANNON_APPROVE_YOURSELF");
        require(msg.sender == tokenOwner || isApprovedForAll(tokenOwner, msg.sender), "NOT_OWNER_AND_NOT_APPROVED_ALL");
        _tokenApprovals[_tokenId] = to;
        emit Approval(tokenOwner, to, _tokenId);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 _tokenId, bytes memory _data) public {
    ...
    function _mint(address to, uint256 _tokenId) internal {
        ...
        require(_tokenOwner[_tokenId] == address(0), "TOKEN_ALREADY_EXISTS");
        require(to != address(0), "TO_IS_INVALID");
    ...
    function _transferFrom(address from, address to, uint256 _tokenId) internal {
        ...
        require(from == ownerOf(_tokenId), "NOT_TOKEN_OWNER");
        require(to != address(0), "TO_IS_INVALID");
     ...
     */
    function _checkOnERC721Received(address from, address to, uint256 _tokenId, bytes memory _data)
    internal returns (bool)
    {
        ...
        bytes4 retval = IERC721Receiver(to).onERC721Received(msg.sender, from, _tokenId, _data);
...
contract ERC721Enumerable is ERC165, ERC721 {
     ...
     */
    function _transferFrom(address from, address to, uint256 _tokenId) internal {
        super._transferFrom(from, to, _tokenId);
     ...
     */
    function _addTokenToOwnerEnumeration(address to, uint256 _tokenId) private {
        _ownedTokensIndex[_tokenId] = _ownedTokens[to].length;
     ...
     */
    function _removeTokenFromOwnerEnumeration(address from, uint256 _tokenId) private {
...
contract MyERC721Mintable is ERC721Metadata {
    ...
    function mint(address to, uint256 _tokenId) public onlyOwner whenNotPaused returns (bool){
        super._mint(to, _tokenId);
